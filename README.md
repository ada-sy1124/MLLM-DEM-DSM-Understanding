...
* **左半场 ($P_{\mathrm{left}}$):** $\mathrm{Mask} = [0, 0, x_{\min}, H]$ *(使用 $x_{\min}$ 保证严格切分出锚点左侧)*
* **右半场 ($P_{\mathrm{right}}$):** $\mathrm{Mask} = [x_{\max}, 0, W, H]$
* **上半场 ($P_{\mathrm{above}}$):** $\mathrm{Mask} = [0, 0, W, y_{\min}]$
* **下半场 ($P_{\mathrm{below}}$):** $\mathrm{Mask} = [0, y_{\max}, W, H]$
...
* **Query:** "Find the $S$ $P_{\mathrm{base}}$ the $A$."
* **Oracle 数据结构:**

  * `target_box`: 目标 $S$ 的真实坐标。
  * `anchor_box`: 锚点 $A$ 的真实坐标。
  * `valid_mask`: 基于 $P_{\mathrm{base}}$ 和 $A$ 计算的神谕蒙版。

#### B. 干预组 1：谓词翻转 (Flip - 核心反事实检验)

* **【极度严苛过滤】：** 遍历图中所有名称为 $S$ 的干扰实体 $S'$。若存在至少一个 $S'$，其几何中心完全落在反向谓词 $P_{\mathrm{flip}}$ 的 $\mathrm{Mask}$ 内，则将其设为反事实目标。**若不存在，该组数据连同 Base 一起直接丢弃！** (保证有物理交集且有实体承接)。
* **Oracle 数据结构:**

  * `target_box`: 对称目标 $S'$ 的真实坐标。
  * `anchor_box`: 继承 Base。
  * `valid_mask`: 基于 $P_{\mathrm{flip}}$ 的反向蒙版。
...
* **生成规则：** 查询 Scene Graph，寻找该目标 $S$ 连向的另一个锚点 $A_{\mathrm{new}}$（例如同一辆车，既在房子左边，又在马路上）。
* **Oracle 数据结构:**

  * `target_box`: 依然是 Base 的那辆车 (目标不变)。
  * `anchor_box`: $A_{\mathrm{new}}$ 的真实坐标 (马路)。
  * `valid_mask`: 根据新锚点 $A_{\mathrm{new}}$ 重新计算的新蒙版。
...
$$
\mathbb{I}\left(
\mathrm{IoU}*{\mathrm{base_ans}}
\ge \tau
\right)
\land
\mathbb{I}\left(
\mathrm{IoU}*{\mathrm{para_ans}}
\ge \tau
\right)
$$
...
重写数据加载逻辑。同一个 `contrast_id` 下的 $(Query_{\mathrm{base}}, Query_{\mathrm{interv}})$ 必须被强制打包进同一个 Mini-Batch 送入模型。这使得在计算 Reward 时，两者的输出轨迹可以产生交叉物理碰撞。

### 3.3 无懈可击的成对奖励公式 (The Bulletproof Reward Math)

在 PyTorch 中实现以下三个奖励算子（Reward Operators）：

#### 🛡️ 目标 1：稠密区域探索先验 ($R_{\mathrm{region}}$) - 解决稀疏性与防作弊网络
...
*(极其精妙的设计：如果模型为了骗取 shift 分数，故意输出互不相交的无效区域（如天空），由于它没命中神谕掩码 $r_{\mathrm{cov}}$ 极低，它的 shift 得分也会瞬间归零！作弊漏洞彻底焊死。)*

#### 🛡️ 目标 2：条件锚点一致性 ($R_{\mathrm{anchor}}$) - 保证参照物智商
...
#### 🛡️ 目标 3：终极神谕命中 ($R_{\mathrm{final}}$) - 守住评测底线
...
**GRPO 参数更新：** 整合总奖励 $R_{\mathrm{total}} = w_1 R_{\mathrm{region}} + w_2 R_{\mathrm{anchor}} + w_3 R_{\mathrm{final}}$。基于配对 GRPO 组内方差计算优势（Advantage），结合 KL 散度约束，爬升策略网络梯度。
...
### 2. Ablation 1: 稠密先验的绝对必要性 (The Necessity of $R_{\mathrm{region}}$)

* **操作：** 去掉 `<think>` 和 $R_{\mathrm{region}}$，只用最终结果 $R_{\mathrm{final}}$ 进行稀疏 RL。
* **结果：** 模型 Loss 剧烈震荡，无法收敛，指标停留在起点。**结论：** 在极大的连续像素动作空间里，直接对齐目标是死路一条，引入行为中间态的稠密物理约束是走通 RL 的绝对基石。

### 3. Ablation 2: 交叉排斥机制的必要性 (The Necessity of Paired $r_{\mathrm{shift}}$)
